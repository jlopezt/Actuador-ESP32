#define COLOR_TEXTO TFT_SKYBLUE
...
#define TOP_ARRIBA_SECUENCIADOR 26
...
//#define ALTO_RECUADROS (((SECUENCIADOR_H-TOP_ARRIBA_SECUENCIADOR)/ZONAS)-ARRIBA_RECUADROS) //Ya esta descontado la separacion entre los recuadros
#define ALTO_RECUADROS (((SECUENCIADOR_H-TOP_ARRIBA_SECUENCIADOR)/secuenciador.getNumPlanes())-ARRIBA_RECUADROS) //Ya esta descontado la separacion entre los recuadros
...
void pantallaSecuenciador(void){
  ...
  for(uint8_t zona=0;zona<zonas;zona++){
    face.drawString(secuenciador.getNombrePlan(zona), 0, TOP_ARRIBA_SECUENCIADOR+(ALTO_RECUADROS+ARRIBA_RECUADROS)*zona+(ALTO_RECUADROS/2));

    /*face.drawRect(LADO_IZQ_RECUADROS,TOP_ARRIBA_SECUENCIADOR+(ALTO_RECUADROS+ARRIBA_RECUADROS)*zona,
    ...
    if(secuenciador.getEstadoPlan(zona)==ESTADO_ACTIVO) face.fillRect(LADO_IZQ_RECUADROS+ANCHO_LINEA_RECTANGULO+(ANCHO_RECUADROS-2*ANCHO_LINEA_RECTANGULO)/2,
                                TOP_ARRIBA_SECUENCIADOR+(ALTO_RECUADROS+ARRIBA_RECUADROS)*zona+ANCHO_LINEA_RECTANGULO,
...
/******************************************* Pantalla Maquina de Estados *****************************************/
#define MAQESTADOS_W 200
#define MAQESTADOS_H 200

#define MAQESTADOS_X_POS (SCREEN_WIDTH-SECUENCIADOR_W)/2 //10
#define MAQESTADOS_Y_POS (SCREEN_HEIGHT-SECUENCIADOR_H)/2 //10

#define TOP_ARRIBA_MAQESTADOS 26
...
void pantallaMaqEstados(void){
  ...
  // Create the clock face sprite
  //face.setColorDepth(8); // 8 bit will work, but reduces effectiveness of anti-aliasing
  face.createSprite(MAQESTADOS_W, MAQESTADOS_H);
  ...
  //Estados y salidas  
  face.setTextDatum(ML_DATUM);
  face.drawString(String("Estado: ") + maquinaEstados.getNombreEstadoActual(), MAQESTADOS_W, TOP_ARRIBA_MAQESTADOS/2);
...
#define CLOCK_R     200.0f / 2.0f   //127.0f / 2.0f // Clock face radius (float type)
...
#define CLOCK_X_POS   (SCREEN_WIDTH-FACE_W)/2 //10
#define CLOCK_Y_POS   (SCREEN_HEIGHT-FACE_H)/2 //10
...
void syncTime(void){
  time_secs = hora() * 3600 + minuto() * 60 + segundo(); // Update the clock time
  }
...
static void renderFace(float t) {
  ...
  face.drawString("Domoticae", CLOCK_R, CLOCK_R * 0.66); //0.75);
  ...
  */
  face.pushSprite(CLOCK_X_POS, CLOCK_Y_POS, TFT_TRANSPARENT);  
}
