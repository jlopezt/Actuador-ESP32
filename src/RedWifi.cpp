...
//******************************Parte especifica del json a leer********************************
  if (doc.containsKey("wifiIP")) wifiIP.fromString((const char *)doc["wifiIP"]); 
  if (doc.containsKey("wifiGW")) wifiGW.fromString((const char *)doc["wifiGW"]);
  if (doc.containsKey("wifiNet")) wifiNet.fromString((const char *)doc["wifiNet"]); 
  if (doc.containsKey("wifiDNS1")) wifiDNS1.fromString((const char *)doc["wifiDNS1"]);
  if (doc.containsKey("wifiDNS2")) wifiDNS2.fromString((const char *)doc["wifiDNS2"]);
  if (doc.containsKey("mDNS")) mDNS=String((const char *)doc["mDNS"]);
...
String generaJsonConfiguracionWifi(String configActual, String ssid, String password){
  ...
  DynamicJsonDocument nuevoDoc(128);
...
/*
  ...
  nuevoDoc["wifiIP"] = cadIp; 
  nuevoDoc["wifiGW"] = cadGw; 
  nuevoDoc["wifiNet"] = cadNet; 
  nuevoDoc["wifiDNS1"] = cadDNS1; 
  nuevoDoc["wifiDNS2"] = cadDNS2; 
*/

  nuevoDoc["wifiIP"] = wifiIP.toString(); 
  nuevoDoc["wifiGW"] = wifiGW.toString(); 
  nuevoDoc["wifiNet"] = wifiNet.toString(); 
  nuevoDoc["wifiDNS1"] = wifiDNS1.toString(); 
  nuevoDoc["wifiDNS2"] = wifiDNS2.toString(); 
  serializeJsonPretty(nuevoDoc,salida);
